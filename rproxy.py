...
"""
    ...
    :using proxy.py by Abhinav Singh
    :license: BSD, see LICENSE for more details.
"""
VERSION = (0, 1)
__version__ = '.'.join(map(str, VERSION[0:1]))
__description__ = 'TCP Reverse Proxy Server in Python'
__author__ = 'Matt Phillips'
__author_email__ = 'anemailaddressiuseforgithub@gmail.com'
__homepage__ = 'https://github.com/greyi9/Reverse-Proxy'
__license__ = 'BSD'
...
class Connection(object):
    ...
    def __init__(self, what):
        self.buffer = b''
        self.closed = False
        self.what = what # server, reverse, or client
    ...
    def recv(self, bytes=8192):
        try:
            ...
            logger.exception("Exception while receiving from connection"+\
                             " %s %r with reason %r") % (self.what, self.conn, e))
            return None
...
class Proxy(multiprocessing.Process):
    ...
    Accepts connection object and acts as a proxy between client and server.
    """
    ...
    def __init__(self, client):
        ...
        self.connection_established_pkt = CRLF.join([
            ...
            b'Proxy-agent: rproxy.py v' + version,
            CRLF
        ])
    ...
    def _process_request(self, data):
        ...
        if self.request.state == HTTP_PARSER_STATE_COMPLETE:
            ...
            try:
                ...
                logger.debug('connection to server %s:%s failed with reason: %s'\
                                                               % (host, port, e))
                self.server = ReverseServer(host, port)
                try:
                    logger.debug('attempting reverse proxy to server %s:%s' % (host, port))
                    self.server.connect()
                    logger.debug('completed reverse proxy to server %s:%s' % (host, port))
                except Exception as e:    
    ...
    def _access_log(self):
        ...
        if self.request.method == b"CONNECT":
            logger.info("%s:%s - %s %s:%s" % (self.client.addr[0], self.client.addr[1],\
                                              self.request.method, host, port))
        elif self.request.method:
            logger.info("%s:%s - %s %s:%s%s - %s %s - %s bytes" % (self.client.addr[0],\
                                              self.client.addr[1], self.request.method,\
                                              host, port, self.request.build_url(),\
                                              self.response.code, self.response.reason,\
    ...
    def _get_waitable_lists(self):
        ...
        if self.server and not self.server.closed and self.server.has_buffer():
            logger.debug("connection to server exists and pending server buffer found,"+\
        ...
        while True:
            ...
            if self.client.buffer_size() == 0:
                if self.response.state == HTTP_PARSER_STATE_COMPLETE:
                    logger.debug('client buffer is empty and response state'+\
                                 'is complete, breaking')
                    break
                
                if self._is_inactive():
                    logger.debug('client buffer is empty and maximum'+\
    ...
    def run(self):
        ...
        try:
            ...
            logger.exception('Exception while handling connection %r with reason %r'\
                                                             % (self.client.conn, e))
        finally:
            logger.debug("closing client connection with pending client buffer size %d bytes"\
                                                                  % self.client.buffer_size())
            self.client.close()
            if self.server:
                logger.debug("closed client connection with pending server buffer size %d bytes"\
...
def main():
    parser = argparse.ArgumentParser(
        description='rproxy.py v%s' % __version__,
        epilog='Report issues at: %s/issues/new' % __homepage__
    )
    ...
    parser.add_argument('--listener_ip', default='0.0.0.0', help='Default: 0.0.0.0')
    parser.add_argument('--listener_port', default='1337', help='Default: 1337')
